...
build_one (function_builder &b, const function_group_info &group,
	   unsigned int oi, unsigned int pi, const vector_type_pair &types)
{
  if (!(*group.shape)->legitimate_shape_p (types))
    return;
  ...
  function_instance function_instance (group.base_name, *group.base,
				       *group.shape, group.ops[oi], types,
				       group.preds[pi]);
  tree return_type = (*group.shape)->get_return_type (types);
	     ...
	     vector_type_index type)
{
  if (relation == RELATION_ONE_ONE)
    {
      vector_type_pair types = {type, type};
      build_one (b, group, oi, pi, types);
    }
  else
    {
      for (unsigned int ti = 0; group.types[ti][1] != NUM_VECTOR_TYPES; ++ti)
	{
	  vector_type_pair types = {type, group.types[ti][1]};
	  build_one (b, group, oi, pi, types);
	}
    }
}
...
/* Add a function instance for every operand && type && predicate
   ...
   suffix from operand_suffixes && mode suffix from mode_suffixes && predication
	   ...
	   const function_group_info &group)
{
  for (unsigned int oi = 0; group.ops[oi] != NUM_OP_TYPES; ++oi)
    for (unsigned int pi = 0; group.preds[pi] != NUM_PRED_TYPES; ++pi)
      for (unsigned int ti = 0; group.types[ti][0] != NUM_VECTOR_TYPES; ++ti)
	build_group (b, relation, group, oi, pi, group.types[ti][0]);
}
...
/* Configuration intruction class.  */
struct configure_def : public one_one_base
{
  tree get_return_type (const vector_type_pair) const override
  {
		  ...
		  bool overloaded_p) const override
  {
    ...
    b.append_name (mode_suffixes[instance.types[0]][BUILT_IN_VSETVL]);
